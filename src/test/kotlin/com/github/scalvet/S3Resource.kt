...
class S3Resource :QuarkusTestResourceLifecycleManager{
    ...
    private val BUCKET_NAME = "quarkus.test.bucket"
    ...
    override fun start(): kotlin.collections.Map<String, String>? {
        ...
        try {
            ...
            client.createBucket(Consumer { b: CreateBucketRequest.Builder -> b.bucket(BUCKET_NAME) })
        } catch (e: Exception) {
        ...
        properties["bucket.name"] = BUCKET_NAME
        return properties
    }
    ...
    private fun endpoint(): String {
        return java.lang.String.format("http://%s:%s", s3.getContainerIpAddress(), s3.getMappedPort(Service.S3.getPort()))
    }
}